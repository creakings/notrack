...
#Description : Loads contents of /var/log/notrack.log into "dnslog" SQL DB
#Author : QuidsUp
#Date Created : 03 October 2016
#Usage : sudo ntrk-parse

#process_dnslog can take a long time to run. In order to prevent loss of DNS queries,
#the log file is loaded into an array, and then immediately zeroed out.
#Processing is done on the array from memory
#Between 04:00 to 04:20 dnslog and weblog tables are trimmed back to $MAXLOGDAYS
#For systems not running 24/7, dnslog and weblog tables are trimmed immediately
...
readonly MAXAGE=5185000                                    #Just over 60 day in seconds
readonly MAXLOGDAYS=60
...
#--------------------------------------------------------------------
# Check Log Age
#   Query timestamp of first value in dnslog table
#   (dnslog table is more likely to record data compared to weblog)
#
# Globals:
...
function check_logage() {
  ...
  log_time=$(mysql -sN --user="$USER" --password="$PASSWORD" -D "$DBNAME" -e "SELECT log_time FROM dnslog ORDER BY log_time LIMIT 1;")
  echo "Oldest log time: $log_time"
  
  if [[ $log_time == "" ]]; then                           #Anything returned? CHECK THIS VALUE
    echo "Nothing in dnslog table"
...
#--------------------------------------------------------------------
# Delete DNS Log Table
#   1. Delete all rows in the dnslog table
#   2. Reset Counter
#
# Globals:
#   USER, PASSWORD, DBNAME
...
#   None
#--------------------------------------------------------------------
function delete_dnslog() {
  mysql --user="$USER" --password="$PASSWORD" -D "$DBNAME" -e "DELETE LOW_PRIORITY FROM dnslog;"
  mysql --user="$USER" --password="$PASSWORD" -D "$DBNAME" -e "ALTER TABLE dnslog AUTO_INCREMENT = 1;"
}


#--------------------------------------------------------------------
# Delete Web Log Table
#   1. Delete all rows in the weblog table
#   2. Reset Counter
#
# Globals:
#   USER, PASSWORD, DBNAME
...
function delete_weblog() {
  mysql --user="$USER" --password="$PASSWORD" -D "$DBNAME" -e "DELETE LOW_PRIORITY FROM weblog;"
  mysql --user="$USER" --password="$PASSWORD" -D "$DBNAME" -e "ALTER TABLE weblog AUTO_INCREMENT = 1;"
}
...
function show_help() {
  ...
  echo "  -a, --age       Check Age of DNS Log table"
  echo "  -h, --help      This Help"
  echo "  -v, --version   Display version number"
  echo "  --delete-dnslog Delete contents of DNS Log table"
  echo "  --delete-weblog Delete contents of Web Log table"
  echo
}
#--------------------------------------------------------------------
#Show Version
function show_version() {
  echo "NoTrack Log Parser v$VERSION"
  echo
}
...
#--------------------------------------------------------------------
# Trim Dnslog Table
#   Delete all rows in the dnslog Table older than $MAXLOGDAYS Days
#
# Globals:
#   USER, PASSWORD, DBNAME, MAXLOGDAYS
# Arguments:
#   None
# Returns:
#   None
...
#--------------------------------------------------------------------
# Trim Weblog Table
#   Delete all rows in the weblog Table older than $MAXLOGDAYS Days
#
# Globals:
#   USER, PASSWORD, DBNAME, MAXLOGDAYS
# Arguments:
...
#--------------------------------------------------------------------
function trim_weblog() {
  mysql --user="$USER" --password="$PASSWORD" -D "$DBNAME" -e "DELETE FROM weblog WHERE log_time < NOW() - INTERVAL $MAXLOGDAYS DAY;"
}
...
#Between 04:00 - 04:20 is time to prune tables
if [[ "$(date +'%H')" == "04" ]]; then
  if [ "$(date +'%M')" -lt 20 ]; then
    trim_dnslog                                            #Trim dnslog table
    trim_weblog                                            #Trim weblog table
    exit 112                                               #No rush to parse log right now
  fi
fi

#Alternate option to anyone not running their system 24/7
check_logage                                               #Is dnslog older than MAXAGE?
...
load_dnslog                                                #Load log file into array
process_dnslog                                             #Process and add log to SQL table
